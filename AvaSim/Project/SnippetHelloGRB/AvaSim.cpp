...
void createStack1(const PxTransform& t, PxU32 size, PxReal halfExtent)
{
	...
	for(PxU32 i=0; i<size;i++)
	{
		for(PxU32 j=0;j<size;j++)
		{
			...
			{
				PxTransform localTm(PxVec3(PxReal(j * 2), PxReal(i * 2 + 1), PxReal(k*2)));
...
void createStack(const PxTransform& t, PxU32 size, PxReal halfExtent)
{
	...
	for (PxU32 i = 0; i<size; i++)
	{
		for (PxU32 j = 0; j<size; j++)
		{		
				PxTransform localTm(PxVec3(PxReal(i * 2 +1), 1, PxReal(j * 2 )));
				PxRigidDynamic* body = gPhysics->createRigidDynamic(t.transform(localTm));
				body->attachShape(*shape);
...
/*
function to create a snowpack filled with small snowgrains
by starting the sceen the grains are sleeping objects

todo:
- initialing a SnowMaterial
- rotation?
*/

void createSnowgrain(const PxTransform& t, PxReal snowgrainsize) 
{
	//basic snowgrain geometry
	PxShape* snowgrain = gPhysics->createShape(PxSphereGeometry(snowgrainsize), *gMaterial);

	//crap	
				//PxTransform localTm(PxVec3(PxReal(i), PxReal(j), PxReal(k)) * snowgrainsize);
				PxTransform localTm(PxVec3(0,0,0));
				PxRigidDynamic* body = gPhysics->createRigidDynamic(t.transform(localTm));
				body->attachShape(*snowgrain);
...
void initPhysics(bool /*interactive*/)
{
	...
	bMaterial = gPhysics->createMaterial(10.0f, 0.5f, 0.1f);

	PxTransform ground_box_position = PxTransform(PxVec3(0.0f, 0.0f, 0.0f),PxQuat(2*PxHalfPi,PxVec3(0.0f,0.0f,1.0f)));
	PxRigidStatic* ground_box = gPhysics->createRigidStatic(ground_box_position);
	ground_box->createShape(PxBoxGeometry(50, 2, 50), *gMaterial);
	gScene->addActor(*ground_box);

	//PxRigidStatic* groundPlane = PxCreatePlane(*gPhysics, PxPlane(0,1,0,0), *gMaterial);
	//gScene->addActor(*groundPlane);

	PxTransform ball_position = PxTransform(PxVec3(0.0f, 30.0f, 0.0f), PxQuat(PxIdentity));
	PxRigidDynamic* ball = gPhysics->createRigidDynamic(ball_position);
	ball->createShape(PxSphereGeometry(2.0f), *bMaterial);
	gScene->addActor(*ball);

	//for(PxU32 i = 0; i<50;i++)
	//createStack(PxTransform(PxVec3(0, 0, stackZ -= 2.0f)),20, 1.0f);
...
void keyPress(const char key, const PxTransform& camera)
{
	switch(toupper(key))
	{
	case 'B':	createStack(PxTransform(PxVec3(0, 0, 0)), 20, 1.0f);								break;
